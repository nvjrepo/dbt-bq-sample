...
stock_movement_last_period as (
    select
        warehouse_id,
        item_id,
        sum(case when issue_receive = 'N' then quantity end) as in_quantity,
        sum(case when issue_receive != 'N' then quantity end) as out_quantity
...
acc_warehouses_this_period as (
    select
        ...
        warehouse_id,
        item_id,
        sum(case when issue_receive = 'N' then quantity end)
        - sum(case when issue_receive != 'N' then quantity end)
        as in_out_quantity
...
week_{{ week_no }}_movement as (
    select acc_warehouses_this_period.*
